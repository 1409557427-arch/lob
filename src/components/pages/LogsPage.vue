<template>
  <div class="logs-page">
    ...
    <div v-if="logsStore.logs.length === 0" class="logs-empty">
      暂无日志记录。
    </div>
      ...
      >
        ...
        <div class="logs-entry__content">
          <div class="logs-entry__header">
            ...
            <span class="logs-entry__time">{{ log.time || log.timestamp || '' }}</span>
          </div>
          <p class="logs-entry__message">{{ log.message || log.content || '' }}</p>
        </div>
      </div>
    </div>
  </div>
